...
<%
    ...
    if ("POST".equalsIgnoreCase(request.getMethod())) {
        if (request.getParameter("insert_subcategory") != null) {
            ...
            filePart = request.getPart("sub_cat_id");

            if (subCategoryName.isEmpty() || filePart == null || parentCategoryId == 0) {
                out.println("<script> alert('Please fill all the fields.'); </script>");
            } else {
                // Save uploaded file
                String fileName = filePart.getSubmittedFileName();
                String uploadPath = application.getRealPath("/photos/" + fileName);
                ...
                try {
                    ...
                    String insertQuery = "INSERT INTO sub_category (sub_cat_name, sub_cat_pic_path, p_cat_id, total_products) VALUES (?, ?, ?, 0)";
                    pst = con.prepareStatement(insertQuery);
                    pst.setString(1, subCategoryName);
                    pst.setString(2, fileName);
                    pst.setInt(3, parentCategoryId);
...
<body class="bg-light">
	<div class="container mt-3">
		...
		<form action="" method="POST" autocomplete="on">
				...
				<label for="parent_category" class="form-label">Select
					...
					<%
                        ...
                        try {
                            ...
                            conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/shop_zone", "root", "");
			...
			<!-- Subcategory Image -->
			<div class="form-outline mb-4 w-50 m-auto">
				<label for="sub_cat_i" class="form-label">Subcategory Image</label>
				<input type="file" name="sub_cat_i" id="sub_cat_i"
					class="form-control" required="required">
			</div>
